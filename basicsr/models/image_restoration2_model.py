...
class ImageRestorationModel2(BaseModel):
    ...
    def __init__(self, opt):
        ...
        load_path = self.opt['path'].get('pretrain_generator', None)
        if load_path is not None:
            print('load generator')
            self.load_network(self.net_g.generator, load_path,
                              self.opt['path'].get('strict_load_g', True), param_key=self.opt['path'].get('param_key', 'params'))
            
        load_path = self.opt['path'].get('pretrain_netE', None)
        if load_path is not None:
            print('load pretrain_netE')
            self.load_network(self.net_g.net_prior, load_path, False, param_key=self.opt['path'].get('param_key', 'params'))
        load_path = self.opt['path'].get('pretrain_priorupsample', None)
        if load_path is not None:
            print('load pretrain_priorupsample')
            self.load_network(self.net_g.prior_upsampling, load_path, False, param_key=self.opt['path'].get('param_key', 'params'))
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                pred = self.net_g(self.lq, self.gt)
    ...
    def get_latest_images(self):
        return [self.gt[0], self.lq[0], self.output[0], self.output1[0]]
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            reblur_img = tensor2img([visuals['reblur']], rgb2bgr=rgb2bgr)
            if 'gt' in visuals:
            ...
            if save_img:
                ...
                if self.opt['is_train']:
                    ...
                    save_img_path = osp.join(self.opt['path']['visualization'],
                                             img_name,
                                             f'{img_name}_{current_iter}_deblur.png')
                    
                    save_gt_img_path = osp.join(self.opt['path']['visualization'],
                                             img_name,
                                             f'{img_name}_{current_iter}_gt.png')

                    save_reblur_path = osp.join(self.opt['path']['visualization'],
                                             img_name,
                                             f'{img_name}_{current_iter}_reblur.png')
                ...
                imwrite(reblur_img, save_reblur_path)
    ...
    def get_current_visuals(self):
        ...
        out_dict['reblur'] = self.output1.detach().cpu()
        if hasattr(self, 'gt'):
