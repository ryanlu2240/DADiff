...
# from basicsr.models.archs.generator_withT import HINet
...
class MSDI2E(nn.Module):
    ...
    #version 1
    def forward(self, x, y, lq=None):
        if lq is not None:
            prior_z = self.net_prior(lq)
        else:
    ...
    # #version 2
    # def forward(self, lq, X0t_gt, original_lq, lq_input=None):
    #     if lq_input is not None:
    #         prior_z = self.net_prior(lq_input)
    #         latent_list_inverse = self.prior_upsampling(prior_z)
    #         latent_list_inverse2 = self.prior_upsampling(prior_z)
    #     else:
    #         prior_z = self.net_prior(lq)
    #         latent_list_inverse = self.prior_upsampling(prior_z)

    #         prior_z2 = self.net_prior(original_lq)
    #         latent_list_inverse2 = self.prior_upsampling(prior_z2)

    #     out = self.generator(lq, latent_list_inverse)

    #     out_inverse = self.inverse_generator(X0t_gt, latent_list_inverse2)
    #     AApy = self.inverse_generator(out, latent_list_inverse)

    #     return out, out_inverse, AApy     
    
    # # version 3 with timestep
    # def forward(self, lq, X0t_gt, original_lq, time_A, time_Ap, lq_input=None):
    #     # if lq_input is not None:
    #     #     prior_z = self.net_prior(lq_input)
    #     # else:
    #     prior_z = self.net_prior(lq)
    ...
    #     out = self.generator(lq, latent_list_inverse, time_Ap)

    #     out_inverse = self.inverse_generator(X0t_gt, latent_list_inverse2, time_A)
    #     AApy = self.inverse_generator(out, latent_list_inverse, time_Ap)
